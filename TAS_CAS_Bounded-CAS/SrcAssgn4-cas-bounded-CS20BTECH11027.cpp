...
bool * waiting;    //Bool array for storing the state of the thread i.e. whether it is waiting or not 
...
void *testCS(void * id){
    ...
    for (int i=0;i<k;i++){
        ...
        waiting[thread_id-1] =true;  //intializing the state of ech thread as waiting 

        bool key = false;            //intializing key as false 

        while(waiting[thread_id-1] && !key){
        ...
        waiting[thread_id-1] = false ; //setting state as not waiting as it had entered the ENTRY section
        ...
        //Finding the next thread which is in waiting state . 
        
        //If we found one then we set it state to not waiting i.e. false, as it will enter the CS next .

        //Else we set the lock to 0 and leave the EXIT section.
...
int main (){
    ...
    output_file.open("Bounded-CAS-Log.txt");  //file for printing the data to .

    waiting = new bool [n];  //memory for waiting array .

    //Intializing waiting array that all the threads are not waiting i.e. false .
    ...
    cout << "The threads are being accessing the CS now once the program finishes it's execution you may check the Bounded-CAS-Log.txt file for the output\n";

    output_file<<"Bounded CAS-ME Output:\n";
    ...
    cout <<"Check the Bounded-CAS-Log.txt file for the output\n";
