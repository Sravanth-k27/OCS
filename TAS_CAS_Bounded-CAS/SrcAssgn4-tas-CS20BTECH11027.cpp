...
atomic_flag lock = ATOMIC_FLAG_INIT; //atomic lock for test and set instruction .
...
void *testCS(void * id){
    ...
    for (int i=0;i<k;i++){
        ...
        atomic_flag_clear(&lock); //freeing the lock so that it will available to other threads .
...
int main (){
    ...
    output_file.open("TAS-Log.txt"); //file for printing the data to .

    cout << "The threads are being accessing the CS now once the program finishes it's execution  you may check  TAS-Log.txt file for the output\n";

    output_file<<"TAS-ME Output:\n"; 
    ...
    cout <<"Check the TAS-Log.txt file for the output\n";
