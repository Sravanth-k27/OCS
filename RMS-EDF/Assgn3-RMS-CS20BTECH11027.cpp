...
//This function caluclates the position of the process which is the ready queue & has high priority (low periodic time)
...
int min_process_id (int dead_line[],int remaining[]){
    ...
    for (int i=0;i<no_processes;i++){
        ...
        if (remaining[i]>0 && remaining[i]<=dead_line[i]){
            ...
            //This if statement gives the position of the process with less period.
    ...
    return a; //returning the position of the low periodic process.

    //Now the next process to be executed has pid in pid[a].

}

//This function implements the Rate monotonic scheduling algorithm.

//Here the input is the total time in which the processes are being scheduled.

void RMS(int total_time)
{  
    //creating files RMS-log.txt & RMS-stats.txt

    ofstream RMS_log ( "RMS-log.txt"); 
    ofstream RMS_stats("RMS-stats.txt");
    
    //this prints out the details of the processes that are to be scheduled to RMS-log.txt file.

    for (int i=0;i<no_processes;i++){
        RMS_log<<"Process P"<<i+1<<":processing time="<<process_time[i]<<";deadline:"<<period_time[i]<<";period:"
    ...
    {   
        for(int j=0;j<no_processes;j++)
        {
            ...
            if(i%period_time[j]==0 && no_iterations[j]<=no_times[j])
            {
                ...
                if(remaining_time[j]>0 && no_iterations[j]>0)
                {
                    ...
                    RMS_log<<"The process P"<<pid[j]<<" had missed the deadline at "<<i<<"\n";
        ...
        for(int j=0;j<no_processes;j++)
        {
            ...
            //The deadline will be again the period after each ietarion (mentioned in line 113).
        ...
        if(before!=after)
        {
            ...
            {
                RMS_log<<"Process P"<<pid[before]<<" finishes execution at time "<<i<<"\n";
            }
            ...
            {
               RMS_log<<"Process P"<<pid[before]<<" is preemepted by Process P"<<pid[after]<<" at time "<<i<<
            ...
            if(after!=-1)
            {
                if (remaining_time[after]==process_time[after]-1){
                    ...
                    RMS_log<<"Process P"<<pid[after]<<" Starts execution at time "<<i<<"\n";

                }
                else {

                   RMS_log<<"Process P"<<pid[after]<<" resumes execution at time "<<i<<"\n";
    ...
    RMS_stats<<"No of processes that came into system: "<<total_times<<"\n";

    RMS_stats<<"No of processes that are successfully completed: "<<total_times-total_missed<<"\n";

    RMS_stats<<"No of processes that missed their deadlines: "<<total_missed<<"\n";

    for (int i=0;i<no_processes;i++){

        RMS_stats<<"Process P"<<pid[i]<<" had missed the deadline for "<<missed[i]<<" times\n";

    }
    for (int i=0;i<no_processes;i++){

        RMS_stats<<"Average waiting time for Process P"<<pid[i]<<" : "<<average_waiting_time[i]<<" milliseconds\n";
    }

    RMS_stats<<"Total average waiting time: "<< total_average_waiting_time<<" milliseconds\n";
...
int main() {
    ...
    cout<<"\nCheck the RMS-log.txt & RMS-stats.txt files for the output\n";
