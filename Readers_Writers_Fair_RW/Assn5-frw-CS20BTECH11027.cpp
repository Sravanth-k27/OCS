...
sem_t mutex , rw_mutex , queue;  //semaphores
...
void *writer(void * id){
    ...
    for (int i=0;i<k_w;i++){
        ...
        sem_wait(&queue); //adding the thread to the service queue handled by semaphore queue.

        sem_wait(&rw_mutex); //acquring semaphore for the resource

        sem_post(&queue); //removing the thread from the service queue handled by semaphore queue.
        ...
        random.seed(thread_id*n_w+i); //seed initilization
...
void *reader(void* id){
    ...
    for (int i=0;i<k_r;i++){
        ...
        sem_wait(&mutex);  //semaphore for incrementing read_count
        ...
        sem_post(&mutex); //releasing semaphore so that others can access.

        //CRITICAL section

        random.seed(thread_id*n_r+i);  //seed intialization 
        ...
        random.seed(thread_id*n_r+i); //seed intilization

        usleep(rem_time(random)*1e3); //simulation of REMIANDER section
    }
...
int main(){
    ...
    output_file_1.open("FRW-log.txt");
    ...
    cout << "The Reader & Writer threads are being accessing the CS.Once the program finishes it's execution check the FRW-log.txt , Average_time.txt files for the output\n";
    ...
    cout <<"Check the FRW-log.txt & Average_time.txt files for the output\n";
